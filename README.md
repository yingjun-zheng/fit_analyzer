...
- **纯桌面图形界面**（原生窗口），**批量导入**码表导出的 `.fit` 文件（iGPSPORT / Garmin / 行者等主流设备），SQLite 本地存储，支持**拖拽 .fit 文件进窗口直接导入**；还能**监控文件夹自动导入**——码表导出目录里出现新文件自动入库并托盘提示
...
- **装备维护管家**：链条/碟片/把带等消耗件台账，按活动里程自动累计使用量，达到预期寿命在月度页提醒更换（支持保养归零、退役）；**多车管理**——每次骑行可标记所骑车辆，装备挂到对应车辆后**只统计该车里程**，精确适配多车用户
- **通勤标记与里程拆分**：活动列表右键标记通勤，月度统计与 Excel 报表自动区分通勤/训练里程
...
- **AI 训练助手（浮窗侧栏）**：右侧常驻浮窗（Ctrl+B 呼出），一键月度总结 + 自由提问，支持多轮会话、跨月追问、流式输出；10 个数据工具（月度/负荷/对比/体能/装备）自动调用，还能让 AI 直接干活（设 FTP/年度目标/添加装备/标记通勤/标记骑行车辆，执行前弹确认框）；**选中活动即锚定上下文**——提问自动围绕这次骑行，一键「复盘本次骑行」
...
- **数据导出与备份**：一键导出 **Excel 报表**（活动明细 + 月度汇总双 Sheet，通勤/训练里程拆分）；**数据库一键备份**（在线一致性快照，运行中也能备）
- 内置**日志系统**（滚动文件 + 界面实时查看）
...
1. 打开软件 → 工具栏「**批量导入 FIT 文件**」，多选码表导出的 `.fit`（可一次性选几百个；重复导入自动更新不重复）。也可以直接把 `.fit` 文件**拖进主窗口**批量导入。
   - **懒人方案**：设置 → 自动导入 → 勾选并选择码表导出文件夹，之后每次码表同步出新文件都会**自动导入**（30 秒检查一次，托盘气泡提示）。
...
- 装备里程 = **启用日期以来「所属车辆」的活动里程**自动累计（+ 启用时已有里程；未指定车辆按全部活动计）；达预期寿命 70% 标「关注」、100% 标「建议更换」，月度页自动出现提醒卡
- **多车管理**：添加/编辑装备时可选「所属车辆」（车辆清单来自台账）；每次骑行在活动列表**右键 → 🚲 骑行车辆**标记（或让 AI 助手 `set_vehicle`），装备即按车分开统计——「公路车的链条该换了吗」AI 可按车查询
...
- 菜单栏（**文件 / 工具 / 帮助**）提供全部功能入口与快捷键（导入 Ctrl+O、导出 GPX Ctrl+E、导出 Excel 报表 Ctrl+Shift+E、删除 Del、刷新 F5），设置在「帮助 → 设置」，另有「关于」。
...
│   ├── excel_export.py   # Excel 报表导出（活动明细+月度汇总）+ 数据库备份
│   ├── auto_import.py    # 文件夹监控自动导入（新 FIT 指纹识别）
