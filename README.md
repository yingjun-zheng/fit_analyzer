...
- **纯桌面图形界面**（原生窗口 + 系统托盘），**批量导入**码表导出的 `.fit` 文件（iGPSPORT / Garmin / 行者等主流设备），SQLite 本地存储
- 单条活动显示：**记录时间、平均速度、卡路里、最大速度、平均心率、平均踏频、累计爬升** 等 16+ 项指标
- **速度按公里图表**、**速度区间统计**、**心率统计与心率区间**、**踏频统计与踏频区间**、**海拔统计**、**设备温度统计**
- **记圈（赛段）数据详情**、**活动详情全部字段**
- **运动轨迹图**：固定背景图片 + 轨迹叠加（起终点标记、海拔范围）
- **按月汇总统计**：各月次数/里程/用时/爬升/消耗，跨月柱状图
...
### 打包好的 EXE（推荐）
`dist\骑行FIT数据分析器\骑行FIT数据分析器.exe` 双击运行（无控制台窗口，托盘常驻；关闭窗口会最小化到托盘，退出请用**托盘图标右键 →「退出」**）。
...
2. 左侧按月列出训练记录，点击某月查看**月度汇总**（次数/里程/用时/爬升/消耗 + 跨月柱状图 + 活动列表，双击活动进入详情）。
3. 点击单条活动进入详情（标签页）：
   - **概览**：16+ 项指标卡 + 每公里均速柱状图 + 速度/心率/踏频/海拔全程曲线 + 设备温度曲线
...
```
fit_analyzer/
├── app.py                # 入口（PySide6，托盘；含 --selftest 离屏自检）
├── gui/
