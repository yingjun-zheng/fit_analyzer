...
路书文件里 `file_id.garmin_product` 的值为 `65534`（即 `Garmin Connect` 云端服务，而非任何具体码表），`product_name` 为空，因此设备栏只能显示「Garmin（佳明）」，**无法定位到具体码表型号**；且路书不含 `max_speed` 字段，故最大速度显示「—」。这是数据本身的限制，如需码表型号与最大速度，请导入佳明码表（Edge 系列）直接记录的**活动文件**。
